...
void Labirint::draw_queue(QLabel* lab[][100], Lab* Lab){
    while (!Lab->draw_queue.isEmpty()){
        int* p = Lab->draw_queue.dequeue();
        int i = p[0];
        int j = p[1];
        int k = p[2];
        delete[] p;
        if (k == 1){
            lab[i][j]->setStyleSheet("background-color: blue");
            Lab->mySleep();
        } else
            if (k == 2){
                lab[i][j]->setStyleSheet("background-color: yellow");
            } else
                if (k == 3){
                    lab[i][j]->setStyleSheet("background-color: red");
                } else
...
void Labirint::FindClicked(){
    ...
    if (find1Box->checkState() + find2Box->checkState() + find3Box->checkState()<4){
        ...
        if (find1Box->checkState() == 2){
            ...
            timer.start();
            Lab1->find1((starti-1)/2, (startj-1)/2, (finishi-1)/2, (finishj-1)/2, 0);
            ...
            if (find2Box->checkState() == 2){
                ...
                timer.start();
                Lab1->find2((starti-1)/2, (startj-1)/2, (finishi-1)/2, (finishj-1)/2);
                lab1Info->setText("Путь найден за " + QString::number(timer.nsecsElapsed()/1000000000)+ ":"
                                                    + QString::number(timer.nsecsElapsed()%1000000000/1000000)+ ":"
                                                    + QString::number(timer.nsecsElapsed()%1000000/1000) + "");
                stream << "BFS ";
